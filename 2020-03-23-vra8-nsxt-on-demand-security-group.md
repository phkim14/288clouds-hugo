...
2. Select "Create an on-demand security group" in the network profile.
...
5. Save the network profile. 

### Create and Configure Blueprint
6. Go to "Blueprints" and Click "+ NEW" to create a new blueprint. (or you can choose to use an existing blueprint that uses existing networks).
7. Give a name to the blueprint and choose a project.
8. Drag on a Cloud Agnostic Machine and a NSX Network onto the canvas. <b>Note that you do not need to add the Security Group object to the blueprint.</b>
...
12. For the network, choose `networkType`. Choose `private` or `outbound` for `networkType`. 
{{<image src="step14.png" linked="true">}}
13. Click "TEST".
14. Click "DEPLOY" to create a new deployment.
15. Give it a deployment name, choose "Current Draft", the cick "DEPLOY".

### Verify Deployment
16. Once deployed, log into NSX-T UI and go to "Advanced Networking & Security" > "Inventory" > Groups". You will see that a new security group has been created with the name starting with "isolation-securitygroup".
{{<image src="step17.png" linked="true">}}
17. Go to "Security" > "Distributed Firewall".
18. You will see that a new firewall section has been created for the security group that has just been created. If you set the `networkType` in the blueprint as `private`, you will see that the rules reject all inbound and outbound traffic. If you set the `networkType` as `outbound`, the rules will allow outbound traffic and reject only the inbound traffic.
